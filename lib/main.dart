import 'package:flutter/material.dart';
import '/pages/home_page.dart';

void main() => runApp(const MyApp());


class MyApp extends StatelessWidget {
  const MyApp({Key? key}) : super(key: key);
  
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Palash Flutter Assignment',
      theme: ThemeData(
        primarySwatch: CustomColors.red,
      ),
      home: const HomePage(),
    );
  }
}

class CustomColors {

  static const red = MaterialColor( 
    0xffd50000,
    <int, Color>{ 
      50:   Color(0xffea8080),
      100:  Color(0xffe66666),
      200:  Color(0xffe24d4d),
      300:  Color(0xffdd3333),
      400:  Color(0xffd91a1a),
      500:  Color(0xffd50000),
      600:  Color(0xffc00000),
      700:  Color(0xffaa0000),
      800:  Color(0xff950000),
      900:  Color(0xff800000),
    }, 
  );

}

// MaterialColor createMaterialColor(final Color color) {
//   List strengths = <double>[.05];
//   final swatch = <int, Color>{};
//   final int r = color.red, g = color.green, b = color.blue;

//   for (int i = 1; i < 10; i++) {
//     strengths.add(0.1 * i);
//   }
//   strengths.forEach((strength) {
//     final double ds = 0.5 - strength;
//     swatch[(strength * 1000).round()] = Color.fromRGBO(
//       r + ((ds < 0 ? r : (255 - r)) * ds).round(),
//       g + ((ds < 0 ? g : (255 - g)) * ds).round(),
//       b + ((ds < 0 ? b : (255 - b)) * ds).round(),
//       1,
//     );
//   });
//   return MaterialColor(color.value, swatch);
// }